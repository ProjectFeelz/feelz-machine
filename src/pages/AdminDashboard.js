...
import {
  ...
  UserX, Crown, MoreVertical, X, Music, Mail, Calendar, Trophy, Zap
} from 'lucide-react';
  ...
  return (
    <div className="pt-14 md:pt-0 pb-32 px-4 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {[
          { label: 'Competitions', icon: Trophy, path: '/admin/competitions', color: 'text-yellow-400' },
          { label: 'Engagement',   icon: Zap,    path: '/admin/engagement',   color: 'text-purple-400' },
        ].map(({ label, icon: Icon, path, color }) => (
          <button key={label} onClick={() => navigate(path)}
            className="flex items-center space-x-2 px-3 py-2.5 bg-white/[0.03] rounded-xl border border-white/[0.06] hover:bg-white/[0.06] transition text-left">
