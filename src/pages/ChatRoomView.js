...
import useKeyboardInset, { useIosViewportPin, useKeyboardOpen } from '../hooks/useKeyboardInset';
...
export default function ChatRoomView() {
  ...
  // null on everything that is not an iPhone or iPad, so Android keeps the
  // behaviour below unchanged. See the hook for why iOS needs its own.
  const iosPin = useIosViewportPin();
  // Used to decide whether to allow for the iPhone home bar under the composer.
  ...
  return (
    <div
      ...
      // On an iPhone the whole room is pinned to the visible area instead of
      // being measured and resized. iosPin is null anywhere else, so Android
      // and desktop keep exactly the behaviour described above.
      style={iosPin || {
    ...
    >
            ...
            <div className="w-9 h-9 rounded-xl overflow-hidden flex items-center justify-center flex-shrink-0"
              ...
              {isBugRoom
                ...
                : room.artists?.profile_image_url
                  ? <img src={coverUrl(room.artists.profile_image_url, 120)} alt="" className="w-9 h-9 object-cover" />
      ...
      {isBugRoom ? (
        ...
        <>
            ...
            <div ref={scrollBoxRef} onScroll={handleScroll}
              ...
              <div className="flex items-center space-x-2 px-2 py-2 mb-2 border-b border-white/[0.04]">
                <div className="w-7 h-7 rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0"
                  ...
                  {room.artists?.profile_image_url
                    ? <img src={coverUrl(room.artists.profile_image_url, 400)} alt="" className="w-7 h-7 object-cover" />
                    ...
                    {!sameSender ? (
                      <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-purple-600/30 to-blue-600/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                        {msg.artist?.profile_image_url
                          ? <img src={coverUrl(msg.artist.profile_image_url, 120)} alt="" className="w-8 h-8 rounded-full object-cover" />
          ...
          {isMember ? (
            <div
              className="px-3 pt-2 pb-3 border-t border-white/[0.06] bg-black/95 backdrop-blur-xl flex-shrink-0"
              // The home bar on an iPhone with no home button.
              //
              // There is a strip at the bottom of the screen that belongs to
              // the phone, not to us, and the send button was sitting in it.
              // The top bar of this room already allows for the notch; the
              // bottom allowed for nothing.
              //
              // Only while the keyboard is CLOSED. With the keyboard up, that
              // strip is covered by the keyboard and iOS keeps reporting a
              // value for it, so allowing for it as well leaves an obvious gap
              // between the composer and the keys.
              style={{ paddingBottom: keyboardOpen ? 12 : 'max(env(safe-area-inset-bottom, 0px), 12px)' }}>
                        ...
                        <button key={m.user_id} onClick={() => insertMention(m)}
                          ...
                          <div className="w-7 h-7 rounded-full overflow-hidden bg-white/10 flex-shrink-0 flex items-center justify-center">
                            {m.artists.profile_image_url
                              ? <img src={coverUrl(m.artists.profile_image_url, 400)} alt="" className="w-full h-full object-cover" />
