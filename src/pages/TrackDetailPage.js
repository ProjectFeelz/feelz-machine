  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let { data: t } = await supabase
        .from('tracks')
        .select('*, artists!tracks_artist_id_fkey(id, artist_name, slug, profile_image_url, is_verified, total_streams, user_id), albums(id, title, cover_artwork_url)')
        .eq('slug', slug)
        .maybeSingle();
      if (!t) {
      ...
      <div className="relative">
        ...
        <div className="relative z-10 px-6 pb-8 flex flex-col items-center text-center">
          ...
          <div className="flex items-center space-x-5 mt-4">
            ...
            <div className="w-px h-6 bg-white/10" />
            <button className="text-center" onClick={() => setShowComments(true)}>
              <p className="text-base font-black text-white">{fmt(commentCount)}</p>
