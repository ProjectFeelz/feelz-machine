...
const location = useLocation();
  ...
  const [addedTo, setAddedTo] = useState({});  const [recommendedTracks, setRecommendedTracks] = useState([]);  const [similarArtists, setSimilarArtists] = useState([]); const [thought, setThought] = useState(null); const [highlightedTrackId, setHighlightedTrackId] = useState(null);
...
const { data: thoughtData } = await supabase
  ...
  .maybeSingle();
if (thoughtData) setThought(thoughtData);

// Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;            if (a && !artistMap[a.id]) artistMap[a.id] = { ...a, matchCount: 0 };            if (a) artistMap[a.id].matchCount++;          });          const sorted = Object.values(artistMap)            .sort((a, b) => b.matchCount - a.matchCount || b.total_streams - a.total_streams)            .slice(0, 6);          setSimilarArtists(sorted);        }      }    }    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;            if (a && !artistMap[a.id]) artistMap[a.id] = { ...a, matchCount: 0 };            if (a) artistMap[a.id].matchCount++;          });          const sorted = Object.values(artistMap)            .sort((a, b) => b.matchCount - a.matchCount || b.total_streams - a.total_streams)            .slice(0, 6);          setSimilarArtists(sorted);        }    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;            if (a && !artistMap[a.id]) artistMap[a.id] = { ...a, matchCount: 0 };            if (a) artistMap[a.id].matchCount++;          });          const sorted = Object.values(artistMap)            .sort((a, b) => b.matchCount - a.matchCount || b.total_streams - a.total_streams)            .slice(0, 6);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;            if (a && !artistMap[a.id]) artistMap[a.id] = { ...a, matchCount: 0 };            if (a) artistMap[a.id].matchCount++;          });          const sorted = Object.values(artistMap)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;            if (a && !artistMap[a.id]) artistMap[a.id] = { ...a, matchCount: 0 };            if (a) artistMap[a.id].matchCount++;    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};          simTrackData.forEach(t => {            const a = t.artists;    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);        if (simTrackData) {          const artistMap = {};    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)          .or(orFilter)          .limit(50);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')          .neq('artist_id', artistData.id)          .eq('is_published', true)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase          .from('tracks')          .select('artist_id, artists(id, artist_name, slug, profile_image_url, is_verified, total_streams)')    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {        const orFilter = allTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(',');        const { data: simTrackData } = await supabase    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];      const allTags = [...genres, ...moods];      if (allTags.length > 0) {    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {      const genres = [...new Set(artistGenres.map(t => t.genre).filter(Boolean))];      const moods = [...new Set(artistGenres.map(t => t.mood).filter(Boolean))];    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)      .limit(20);    if (artistGenres && artistGenres.length > 0) {    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')      .eq('artist_id', artistData.id)      .eq('is_published', true)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase      .from('tracks')      .select('genre, mood')    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Similar artists: match by genre overlap in tracks    const { data: artistGenres } = await supabase    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);      }    }    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })          .limit(5);        setRecommendedTracks(topData || []);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)          .eq('is_published', true)          .order('engagement_score', { ascending: false })    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')          .select('*, albums(title, cover_artwork_url, price)')          .eq('artist_id', artistData.id)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet        const { data: topData } = await supabase          .from('tracks')    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }      } else {        // Fallback: show top tracks user hasn't streamed yet    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);          setRecommendedTracks(recData || []);        }    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)            .order('engagement_score', { ascending: false })            .limit(5);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)            .or(topTags.map(t => `genre.eq.${t},mood.eq.${t}`).join(','))            .not('id', 'in', `(${listenedIds.join(',')})`)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')            .eq('artist_id', artistData.id)            .eq('is_published', true)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase            .from('tracks')            .select('*, albums(title, cover_artwork_url, price)')    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);        if (topTags.length > 0) {          const { data: recData } = await supabase    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });        const listenedIds = streamData.map(s => s.track_id).filter(Boolean);        const topTags = Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0]);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;          if (m) tagCounts[m] = (tagCounts[m] || 0) + 1;        });    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {          const g = s.tracks?.genre; const m = s.tracks?.mood;          if (g) tagCounts[g] = (tagCounts[g] || 0) + 1;    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks        const tagCounts = {};        streamData.forEach(s => {    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);      if (streamData && streamData.length > 0) {        // Count genre/mood frequency from listened tracks    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)        .eq('user_id', user.id)        .limit(50);    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')        .select('track_id, tracks(genre, mood)')        .eq('tracks.artist_id', artistData.id)    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks      const { data: streamData } = await supabase        .from('streams')    // Fetch recommendations and similar artists    if (user) {      // Get user's streams for this artist's tracks    // Fetch recommendations and similar artists
...
{thought && (
  <div className="px-6 mb-6">
    <div className="rounded-2xl px-5 py-4" style={{ backgroundColor: `${secondaryColor}12`, border: `1px solid ${secondaryColor}25` }}>
      <p className="text-xs font-semibold mb-2" style={{ color: `${secondaryColor}90` }}>💭 Today's Thought</p>
      <p className="text-sm leading-relaxed" style={{ color: `${textColor}90` }}>{thought.content}</p>
      <p className="text-[11px] mt-2" style={{ color: `${textColor}30` }}>
        {new Date(thought.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
      </p>
    </div>
  </div>
)}
...
{tracks.length > 0 && (
  ...
  style={{ backgroundColor: isActive ? `${secondaryColor}15` : highlightedTrackId === track.id ? `${secondaryColor}25` : 'transparent', outline: highlightedTrackId === track.id ? `1px solid ${secondaryColor}50` : 'none' }}>
