...
export default function TrackUploadPanel() {
  ...
  const addTrackToAlbum = async (trackId) => {
    await supabase.from('tracks').update({ album_id: selectedAlbumId }).eq('id', trackId);
    fetchAlbumTracks(selectedAlbumId);
  };
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'albums' && (
        <div className="space-y-6">
          ...
          {albums.length > 0 && (
            <div className="bg-white/[0.03] rounded-xl p-5 border border-white/[0.06] space-y-4">
              <h3 className="text-base font-semibold text-white">Manage Album Tracks</h3>
              <select value={selectedAlbumId}
                onChange={(e) => { setSelectedAlbumId(e.target.value); fetchAlbumTracks(e.target.value); }}
                className="w-full px-3 py-2.5 bg-white/[0.06] rounded-lg text-white text-sm outline-none">
              ...
              {selectedAlbumId && (
                loadingAlbumTracks
                  ...
                  : <div className="space-y-4">
                      <div>
                        <p className="text-xs text-white/40 mb-2">In this album ({albumTracks.length})</p>
                        <div className="space-y-1.5">
                          ...
                          {albumTracks.length === 0 && <p className="text-xs text-white/20 py-2 text-center">No tracks in this album yet</p>}
                        </div>
                      </div>
                      {unassignedTracks.length > 0 && (
                        <div>
                          <p className="text-xs text-white/40 mb-2">Add from your singles</p>
                          <div className="space-y-1.5">
