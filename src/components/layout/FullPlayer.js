...
// Icons as inline SVGs to avoid adding lucide icons that don't exist
const IconImage = () => (
...
export default function FullPlayer() {
  ...
  // Reset to artwork when track changes if current mode is unavailable
  useEffect(() => {
  ...
  const handleShare = async () => {
    tap();
    const slug = currentTrack.artist_slug || currentTrack.slug || '';
    const url  = slug ? `${window.location.origin}/artist/${slug}` : window.location.href;
    if (navigator.share) {
      try { await navigator.share({ title: currentTrack.title, text: `${currentTrack.title} by ${currentTrack.artist_name}`, url }); }
      catch {}
    } else {
      ...
      {showQueue ? (
        ...
        <>
          ...
          <div className="flex-1 relative flex flex-col items-center justify-center px-8 min-h-0 overflow-hidden">
            ...
            {/* Video layer — fills the area, behind everything when active */}
            {displayMode === 'video' && hasVideo && (
              <div className="absolute inset-0">
                ...
                />
                {/* Scrim so bottom controls stay legible */}
            ...
            {/* ── Mode toggle — bottom of artwork area ── */}
          ...
          <div className="px-8 pb-10 flex-shrink-0">
            ...
            <div className="flex items-center justify-center mt-5 space-x-8">
              <button onClick={handleShare}
                ...
                <span className="text-[10px]">{shared ? 'Copied!' : 'Share'}</span>
              </button>
