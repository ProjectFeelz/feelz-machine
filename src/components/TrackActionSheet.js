...
import {
  ...
  ChevronLeft, ShoppingCart, Lock, PlusCircle, DollarSign,
} from 'lucide-react';
...
export default function TrackActionSheet({ track, artist, onClose }) {
  ...
  const [view, setView] = useState('main'); // 'main' | 'playlists' | 'purchase'
  ...
  useEffect(() => {
    if (!track) return;
    if (user) checkLike();
    // Pre-fill the fan price when the track loads
    if (isPWYW) {
  ...
  useEffect(() => {
    ...
    if (isPWYW && !validateFanPrice()) return; // Don't render until valid amount
    ...
    window.paypal.Buttons({
      ...
      onApprove: async (data) => {
        try {
          ...
          // Trigger the actual download after a short success animation
          setTimeout(async () => {
            try {
  ...
  const validateFanPrice = () => {
    ...
    if (isNaN(val) || val < 0) {
      setFanPriceError('Please enter a valid amount');
      return false;
    }
    if (minimumPrice > 0 && val < minimumPrice) {
      setFanPriceError(`Minimum is $${minimumPrice.toFixed(2)}`);
      return false;
    }
    setFanPriceError('');
    return true;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: track.title,
          text: track.title + ' by ' + (artist?.artist_name || track.artist_name),
          url,
        });
      } catch {}
    } else {
  ...
  // Label for the download button in main view
          ...
          <div className="flex-1 min-w-0">
            ...
            <button
              onClick={() => {
                ...
                if (slug) { navigate(`/artist/${slug}`); onClose(); }
              }}
        ...
        <div className="py-2">
            ...
            <div>
              {showNewPlaylist ? (
                <div className="flex items-center space-x-2 px-4 py-2.5 border-b border-white/[0.05]">
                  ...
                  <button
                    ...
                    className="p-2 text-white/30 hover:text-white/60 transition flex-shrink-0"
                  >
              ...
              {playlists.map(pl => (
                ...
                >
                  ...
                  {addedTo[pl.id] ? <Check className="w-4 h-4 text-green-400" />
                    : addingTo === pl.id ? <Loader className="w-4 h-4 animate-spin text-white/30" /> : null}
                </button>
              ))}
            </div>
          )}
                  ...
                  {isPWYW ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between rounded-xl p-3"
                        ...
                        {minimumPrice > 0 && (
                          <p className="text-xs text-white/30">min $${minimumPrice.toFixed(2)}</p>
                        )}
                      </div>
                      ...
                      {minimumPrice === 0 && (
                        <p className="text-xs text-white/30 text-center">Enter $0 to download free</p>
                      )}
                      <div className="flex space-x-2">
                        {[1, 2, 5, 10].filter(v => v >= minimumPrice).map(v => (
                          <button
                            key={v}
                            type="button"
                            onClick={() => { setFanPrice(v.toFixed(2)); setFanPriceError(''); }}
                      ...
                      {parseFloat(fanPrice) > 0 ? (
                        <>
                          ...
                          {!paypalReady && !purchaseError && (
                            <div className="flex justify-center py-2"><Loader className="w-5 h-5 animate-spin text-white/30" /></div>
                          )}
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await supabase.from('downloads').insert({
                                user_id: user.id, track_id: track.id, amount_paid: 0
                              }).catch(() => {});
                              const { data: { session } } = await supabase.auth.getSession();
                              await downloadTrack(track.id, track.title, session?.access_token);
                              onClose();
                    ...
                    <>
                      <div className="flex items-center justify-between rounded-xl p-3"
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <p className="text-sm text-white/60">Purchase to download</p>
                        </div>
          ...
          {view === 'main' && (
            <>
                ...
                <>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <span className="text-sm text-white/70">{downloadButtonLabel()}</span>
                    </div>
                    {isPWYW && !downloading && (
              ...
              <button
                onClick={() => { navigate(`/artist/${artist?.slug || ''}`); onClose(); }}
