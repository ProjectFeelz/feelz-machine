...
import { Palette, Save, Loader, Upload, Eye, X } from 'lucide-react';
...
export default function ThemeEditor() {
  ...
  const handleRemoveBanner = async () => {
        ...
        setSaving(true);
        try {
                const updates = { banner_image_url: null };
                if (hasTheme) {
                          await supabase.from('artist_themes').update(updates).eq('artist_id', artist.id);
                }
  ...
  const handleRemoveBgImage = async () => {
        ...
        try {
                if (hasTheme) {
                          await supabase.from('artist_themes').update({ background_image_url: null }).eq('artist_id', artist.id);
                }
      ...
      <div>
                         ...
                         {/* Banner Image */}
                                   <div className="space-y-2">
                                     <label className="block text-xs text-white/40">Banner Image (1200x400 recommended)</label>
...
{artist.bio && (
                <p className="text-[10px] leading-relaxed mb-3 max-w-xs" style={{ color: `${theme.text_color}70` }}>
