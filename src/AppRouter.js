...
// ── Redirects new users without a profile to /setup ─────────────────────────
function OnboardingGuard({ children }) {
  ...
  if (user && !artist && !listener) return <Navigate to="/setup" replace />;
  return children;
}
...
function PageTitle({ title, children }) {
  return (
    <>
      <Helmet><title>{title} · Feelz Machine</title></Helmet>
      {children}
    </>
  );
}
...
function MerchConnectCallback() {
  ...
  React.useEffect(() => {
    ...
    if (code && state) navigate(`/artist/${state}?printful_code=${code}`, { replace: true });
          ...
          <PlayerProvider>
            <TierProvider>
              ...
              <Routes>
                ...
                <Route path="/terms-of-use" element={<PageTitle title="Terms of Use"><TermsOfUse /></PageTitle>} />
