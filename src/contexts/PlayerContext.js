...
export function PlayerProvider({ children }) {
  ...
  const logStream = async (trackId) => {
    try {
      ...
      if (!userId) return;
      console.log('Attempting stream log:', { trackId, userId });
      ...
      console.log('Stream result:', streamError ? JSON.stringify(streamError) : 'success');
      ...
      if (track) {
        ...
        const { data: collabs } = await supabase.from('collaborations')
          .select('artist_id').eq('track_id', trackId).eq('status', 'accepted');
        if (collabs?.length) {
    ...
    } catch (err) { console.error('Failed to log stream:', err); }
  };
